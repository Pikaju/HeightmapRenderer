...
namespace math {
	...
	{
		T f = tan(fov * (T)(3.14159265358979323846 / 180.0) / (T)2);
		Mat4<T> m;
		m.set(0, 0, (T)1 / (aspect * f)); m.set(1, 0, 0); m.set(2, 0, 0); m.set(3, 0, 0);
		m.set(0, 1, 0); m.set(1, 1, (T)1 / f); m.set(2, 1, 0); m.set(3, 1, 0);
		m.set(0, 2, 0); m.set(1, 2, 0); m.set(2, 2, -(near - far) / (near - far)); m.set(3, 2, (T)2 * far * near / (near - far));
